...
import ast  # 更安全的替代 eval()

def build_optimizer(cfg, model, contiguous=True):
    import ast
    import copy
    import torch
    from fastreid.solver import build_optimizer as build_optimizer_default
    from fastreid.utils.params import ContiguousParams
    from fastreid.solver.build import get_default_optimizer_params, maybe_add_gradient_clipping, maybe_add_freeze_layer

    # ----------- 👇 新增：解析 SPECIAL_LAYERS，支持 str / list / dict ------------
    raw = cfg.SOLVER.SPECIAL_LAYERS
    special_layer_map = {}

    if isinstance(raw, str):
        try:
            special_layer_map = ast.literal_eval(raw)
        except Exception as e:
            raise ValueError(f"Failed to parse SOLVER.SPECIAL_LAYERS string: {e}")
    elif isinstance(raw, list):
        for item in raw:
            if isinstance(item, str) and "=" in item:
                key, val = item.split("=", 1)
                special_layer_map[key.strip()] = float(val.strip())
            else:
                raise ValueError(f"Invalid SPECIAL_LAYERS item: {item}")
    elif isinstance(raw, dict):
        special_layer_map = raw
    # -----------------------------------------------------------------------------
    ...
    # 处理 SPECIAL_LAYERS 分组
    if special_layer_map:
        new_params = []
        for group in params:
            ...
            for p in group["params"]:
                ...
                for name, param in model.named_parameters():
                    if param is not p:
                        continue
                    for prefix, lr in special_layer_map.items():
                        if name.startswith(prefix):
                            if lr not in special_groups:
                                g = copy.deepcopy(group)
                                g["params"] = []
                                g["lr"] = lr
                                special_groups[lr] = g
                            special_groups[lr]["params"].append(p)
                            matched = True
                            break
                    break
                if not matched:
                    base_group["params"].append(p)

            if base_group["params"]:
                new_params.append(base_group)
            for g in special_groups.values():
                new_params.append(g)
    ...
    opt_class = maybe_add_freeze_layer(
        cfg, maybe_add_gradient_clipping(cfg, getattr(torch.optim, cfg.SOLVER.OPT))
    )
    ...
    if cfg.SOLVER.OPT == "SGD":
        ...
        optimizer = opt_class(params.contiguous() if contiguous else params)

...
class DefaultTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        if getattr(cfg.SOLVER, "GROUP_LR", False):
            for i, group in enumerate(param_wrapper._parameters):
                print(f"Param group {i}: lr = {group['lr']}, num params = {len(group['params'])}")
                for p in group["params"]:
                    names=[n for n, param in model.named_parameters() if param is p]
                    print(f"  param: {names[0] if names else '[unknown]'}")

...
class VisionTransformer_multiview_onebranch(nn.Module):
    ...
    def forward(self, x, camera_id=None):
        ...
        x = self.norm(x)
        weight=self.blocks[11].attn.weight
        visualize_in_forward(img, weight)
...
def visualize_in_forward(img_tensor, attn_weight, save_dir="hotmap_diff"):
    """
    img_tensor: (B, 3, 256, 128)
    attn_weight: (B, 12, 130, 130) -> 0:CLS, 1:View, 2-129:Patches
    目标：可视化 (CLS->Patches) - (View->Patches)
    """
    ...
    current_idx = len([name for name in os.listdir(save_dir) if name.endswith('.jpg')])

    imgs = img_tensor.detach().cpu().numpy()
    weights = attn_weight.detach().cpu().numpy()
    ...
    for i in range(B):
        ...
        # 2. 提取注意力矩阵并计算差值
        avg_attn = weights[i].mean(axis=0)  # (130, 130)

        cls_to_patches = avg_attn[0, 2:]  # (128,)
        view_to_patches = avg_attn[1, 2:]  # (128,)

        # 核心步骤：减去 View Token 的注意力
        diff_attn = cls_to_patches - view_to_patches

        # 3. 归一化差值图
        # 因为差值可能存在负数，我们需要将其映射到 0-1
        # 如果你想只看 CLS 比 View 强的地方，可以用 np.maximum(diff_attn, 0)
        diff_attn = (diff_attn - diff_attn.min()) / (diff_attn.max() - diff_attn.min() + 1e-8)

        # 4. 空间还原与放缩
        mask = diff_attn.reshape(16, 8)
        ...
        save_name = f"{str(current_idx).zfill(5)}.jpg"

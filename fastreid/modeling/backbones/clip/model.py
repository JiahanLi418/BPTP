...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int, vdt=False):
        super().__init__()
        ...
        if vdt:
            ...
            self.positional_embedding = nn.Parameter(scale * torch.randn(h_resolution * w_resolution + 2, width))
        else:
    ...
    def forward(self, x: torch.Tensor, cv_emb = None):
        ...
        if self.vdt:
            x = torch.cat([
                self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype,
                                                               device=x.device),
                self.view_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype,
        ...
        x11=x12=self.transformer.resblocks[:1](x)
        if self.vdt:
            for i in range(12):
...
def build_model(state_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int,vdt):
    ...
    model = CLIP(
        ...
        h_resolution, w_resolution,vdt
    )

    if vit:
        if vdt:
            state_dict["visual.positional_embedding"] = resize_pos_embed(state_dict["visual.positional_embedding"], model.visual.positional_embedding, h_resolution, w_resolution,cls_token_num=2)
        else:
            state_dict["visual.positional_embedding"] = resize_pos_embed(state_dict["visual.positional_embedding"],
                                                                         ...
                                                                         h_resolution, w_resolution, cls_token_num=1)
    else: #RN50
        state_dict["visual.attnpool.positional_embedding"] = resize_pos_embed(state_dict["visual.attnpool.positional_embedding"], model.visual.attnpool.positional_embedding, h_resolution, w_resolution,cls_token_num=1)
...
def resize_pos_embed(posemb, posemb_new, hight, width,cls_token_num):
    ...
    posemb_token, posemb_grid = posemb[:cls_token_num], posemb[1:]
    ntok_new -= 1

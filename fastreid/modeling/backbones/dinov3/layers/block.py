...
class Adapter(nn.Module):
    def __init__(self, dim, reduction=2, act_layer=nn.GELU,drop_rate=0.0):
        super().__init__()
        ...
        nn.init.zeros_(self.up_proj.weight)  # 上投影初始化为0
    ...
    def _forward_list(self, x_list: List[Tensor], rope_list=None) -> List[Tensor]:
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            for x, rope in zip(x_list, rope_list):
                x_attn = x + self.ls1(self.adapter1(self.norm1(x))+self.attn(self.norm1(x), rope=rope))
                x_ffn = x_attn + self.ls2(self.adapter2(self.norm2(x_attn))+self.mlp(self.norm2(x_attn)))
                x_out.append(x_ffn)
            x_ffn = x_out

...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor]) -> List[Dict[str, Tensor]]:
        ...
        target_layers={2, 5, 8, 11}  # 对应 3/6/9/12 层
        saved_intermediate=[]
        ...
        for _, blk in enumerate(self.blocks):
            ...
            if _ in target_layers:
                for x_view in x:
                    patch_tokens=x_view[:, 5:]  # cls + 4 storage token 之后
                    layer_patches.append(patch_tokens)
                saved_intermediate.append(layer_patches)
        all_x = x
        output = []
            ...
            output.append(
                {
                    ...
                    "x_prenorm": x,
                    "masks": masks,
                    "muti_patchs":saved_intermediate,
                }
            )
        return output
    ...
    def forward(self, *args, is_training: bool = False, **kwargs) -> List[Dict[str, Tensor]] | Tensor:
        ...
        if is_training:
            ...
            return torch.cat((
                ret["x_norm_clstoken"].unsqueeze(1),ret["x_norm_patchtokens"]
            ),dim=1),ret["muti_patchs"]
